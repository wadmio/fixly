...
- Commands **Fixly: Scan Current Project** (`fixly.scanCurrentProject`), **Fixly: Show Last Report** (`fixly.showReport`, also the status-bar click target), and **Fixly: Fix Everything & Verify** (`fixly.fixEverything`).
...
- [src/extension.ts](apps/extension/src/extension.ts) is the **guardian** (demo runbook: [docs/extension-demo.md](docs/extension-demo.md)): activates on `workspaceContains:package.json`, runs a baseline scan, then watches the manifests **on disk** via `FileSystemWatcher` (catches editor saves, terminal `npm install`, AI-agent writes — `onDidSaveTextDocument` misses the last two; 1.2s debounce, `fixly.scanOnSave`) plus an **advisory clock** (`fixly.advisoryIntervalMinutes`, default 10, periodic quiet re-scan). Each scan is diffed via core's `snapshotFindings`/`diffFindingSnapshot` (KEV escalations = their own event). New/escalated findings **auto-remediate by default** (`fixly.autoRemediate`; off = a "Fix & Verify" prompt): [src/remediator.ts](apps/extension/src/remediator.ts) rewrites package.json as an **undoable WorkspaceEdit**, runs `npm install`, re-scans to verify, and reports measured MTTR in toasts + a session **activity feed**. Baselines never auto-fix (a toast offers Fix Everything). Status bar shows the **grade** (`Fixly A · 100`; D/F error background, C warning). `scanning`/`remediating` flags prevent overlap and self-triggering.
...
- [src/panel.ts](apps/extension/src/panel.ts) renders a webview report (**Fixly Score card** via `computeGrade` with top fixes + **Grade Forecast**, **⚡ Fix Everything & Verify** button wired to `fixly.fixEverything`, **Guardian activity feed** with MTTR badges, summary cards, findings table with transitive chips + NVD scores, warnings, **Rescan / Copy Summary / Export JSON**); `FixlyPanel.updateIfOpen()` refreshes it after guardian scans without stealing focus. Pure rendering lives in [src/panel-render.ts](apps/extension/src/panel-render.ts) (vscode-free, tested).
...
- Auto-fixing is allowed in exactly two places: **`fixly daemon`** and the **extension guardian** (real-time remediation is the capstone requirement). The shared contract: never auto-fix a baseline, always verify by re-scan before claiming success, and an opt-out must stay available (`--notify-only` / `fixly.autoRemediate: false`). Every other surface stays plan/dry-run first (`fixly fix` requires `--write`; web shows the plan only).
