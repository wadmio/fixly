...
7. **Verdict engine** (powers `check`/vibecheck/MCP/guard surfaces):
   ...
   - [resolution.ts](packages/core/src/resolution.ts) — **resolution engine** (pure, no network/fs): `resolveRemediation(findings, graph)` → a structured `Remediation` per vulnerable package@version: resolution path (`DIRECT` / `TRANSITIVE_OVERRIDE` / `BLOCKED_BY_PARENT` / `NO_FIX_AVAILABLE`), the **minimum safe target version** (max of per-advisory minimal fixes, checked against every dependent's declared range), semver distance (`PATCH`/`MINOR`/`MAJOR`), risk (`low`/`elevated` — MAJOR jumps and overrides are elevated), and a plain-English rationale. Candidates come only from OSV fixed events (no registry enumeration); unparseable ranges never block; `BLOCKED_BY_PARENT` and `NO_FIX_AVAILABLE` are legitimate terminal states, not errors. [lock-graph.ts](packages/core/src/lock-graph.ts) `buildDependencyGraph(lock)` supplies the graph from a v2/v3 package-lock (`dependentsOf` uses nearest-node_modules resolution; v1/absent → null → constraint checks skipped, stated in the rationale). `satisfiesRange` (npm-range subset, null = unknown) lives in [matching.ts](packages/core/src/matching.ts).
   - [remediate.ts](packages/core/src/remediate.ts) — **advice-only fix plan**: `buildRemediationPlan(result, {graph?})` → one action per vulnerable package@version (malware → `remove`, **except a transitive malware advisory that has a published fix → `override` to it** — you can't uninstall a dep you didn't declare, e.g. fsevents <1.2.11; direct → `upgrade`, transitive → `override`), each carrying its `resolution`, a copy-paste command (text for the user/agent — Fixly never runs or applies it), and points recovered, plus a **Grade Forecast** — `after` is literally `computeGrade` on the scan minus the fixed findings, so "D (58) → A (96)" is real arithmetic. Graph-blocked fixes land in `blocked` (advice-only, excluded from `fixableFindings`); no-fix findings land in `unfixable`; both stay in the forecast. Same plan powers `fixly fix`, the web Remediation Plan card, and the extension report/diagnostics.
...
- **`fixly fix [dir]`** ([src/commands/fix.ts](apps/cli/src/commands/fix.ts)) — prints the remediation plan + Grade Forecast as copy-paste advice (`--json` for machines). No `--write`: Fixly analyzes and verifies, never modifies — the user (or their AI agent) applies the commands, then `npm install && fixly vibecheck` verifies the forecast.
...
- [src/panel.ts](apps/extension/src/panel.ts) renders a webview report (**Fixly Score card** via `computeGrade` with top fixes, Grade Forecast line, **Remediation Plan section** — risk-chipped actions with rationale + copy-paste commands, a **Blocked** card naming the parent and range, unfixable list — summary cards, findings table with transitive chips + NVD scores, warnings, **Rescan / Copy Summary / Export JSON**); `FixlyPanel.updateIfOpen()` refreshes it after on-save scans without stealing focus. All remediation display is advice — the report has no apply control.
...
- Keep it tight (student capstone). The extension is the one packaged surface: it ships to the VS Code Marketplace as `fixly.fixly-vscode` (`pnpm --filter fixly-vscode package:vsix`, published with `vsce publish` under the `fixly` publisher). Still no CI/CD gating.
- **Fixly analyzes and verifies, never modifies.** No surface edits files or runs installs — not the extension, not the CLI, not core. Remediation output is precise advice (target versions, override entries, copy-paste commands) for the user or their AI agent to apply; Fixly then verifies the outcome on the next scan. Do not reintroduce apply/write features.
