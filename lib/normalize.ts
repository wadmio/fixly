...
export function normalizeOsvResults(
  ...
  // Debug: log raw OSV shape for the first vuln of each package so you can
  // inspect what fields OSV is actually returning in the dev server terminal.
  if (process.env.NODE_ENV !== "production" && vulns[0]) {
    const { id, severity, database_specific, affected } = vulns[0];
    console.log(`[osv:${packageName}]`, JSON.stringify({
      id,
      severity,
      database_specific,
      first_range: affected?.[0]?.ranges?.[0],
    }, null, 2));
  }
